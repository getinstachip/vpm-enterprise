...
main() {
    ...
    local _bin_name
    case "${_arch}" in
    *windows*) _bin_name="vpm-enterprise.exe" ;;
    *) _bin_name="vpm-enterprise" ;;
    esac
    ...
    *)
    ...
    echo ""
    echo "vpm-enterprise is installed!"
    if ! echo ":${PATH}:" | grep -Fq ":${BIN_DIR}:"; then
        echo "Note: ${BIN_DIR} is not on your \$PATH. vpm-enterprise will not work unless it is added to \$PATH."
    fi
}
...
usage() {
    ...
    echo "\
${_text_heading}vpm-enterprise installer${_text_reset}
Instachip <team@getinstachip.com>
https://github.com/getinstachip/vpm-enterprise

Fetches and installs vpm-enterprise. If vpm-enterprise is already installed, it will be updated to the latest version.
...
download_vpm() {
    ...
    local _releases_url="https://api.github.com/repos/getinstachip/vpm-enterprise/releases/latest"
    local _releases
    case "${_dld}" in
