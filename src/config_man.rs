...
pub fn get_config_path() -> Option<PathBuf> {
    ProjectDirs::from("com", "instachip", "vpm-pro")
...
pub fn create_config() -> Result<()> {
    ...
    let mut config_doc = DocumentMut::new();

    config_doc.insert("user", Item::Table(Table::new()));
    let user_table = config_doc["user"].as_table_mut().unwrap();
    user_table.insert("uuid", Item::Value(Value::from(machine_uid::get().unwrap_or(Uuid::now_v7().to_string()))));
    user_table.insert("license", Item::Value(Value::from("")));
    ...
    fs::write(config_path, format!("# Avoid editing this file!\n{}", config_doc.to_string())).expect("Failed to write config.toml");
    Ok(())
}
